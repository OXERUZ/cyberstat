...
/* ---------------- Home ---------------- */
function Home({ settings, totalVotes, candidateCount, participants, setView }) {
      ...
      <div className="grid md:grid-cols-[1.1fr_0.9fr] gap-10 items-center">
        <div className="glow-in">
          <div className="inline-flex items-center gap-2 text-xs font-display px-3 py-1.5 rounded-full mb-6"
            style={{ background: "rgba(0,224,255,0.08)", border: "1px solid rgba(0,224,255,0.25)", color: "#5FE8FF" }}>
            <RadioTower size={13} />
            {settings.tagline.toUpperCase()}
          </div>
          <h1 className="font-display font-bold leading-[1.05] mb-6" style={{ fontSize: "clamp(2.2rem, 5vw, 3.6rem)", color: "#F2F8FF" }}>
            Sizning ovozingiz —<br />sizning tanlovingiz
          </h1>
          <p className="mb-8 max-w-md leading-relaxed" style={{ color: "#96A6C8", fontSize: "1.05rem" }}>
            O‘zingiz munosib deb bilgan nomzodga ovoz bering va natijalarni real vaqt rejimida kuzating.
          </p>
          <div className="flex flex-wrap gap-3 mb-12">
            <button onClick={() => setView("vote")} disabled={ended}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <Stat label="NOMZODLAR" value={candidateCount} />
            <Stat label="ISHTIROKCHILAR" value={<CountUp value={participants} />} />
            <Stat label="TIZIM HOLATI" value={<StatusPill status={settings.status} compact />} raw />
          </div>
        </div>
        <HudPanel totalVotes={totalVotes} status={settings.status} />
      </div>
    </main>
  );
}
